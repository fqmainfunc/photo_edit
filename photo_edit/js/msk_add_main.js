...
function canva3_toimg() {
	var base64 = document.getElementById("canvas3").toDataURL('image/jpeg'); //将图片数据转为base64.
	image_all.src=base64;
	 
	}
function  canvas3_refresh(){
	var canvas3 = document.getElementById('canvas3');
...
function getLocation(x, y) {
	var canvas3 = document.getElementById('canvas3');
	var bbox = canvas3.getBoundingClientRect();
	return {
...
function draw(obj,start_x,start_y,msk_mhcd,size){
    //绘制第一张图
	var ctx3 = obj.getContext("2d");
    //获取坐边图像的局部坐标的部分像素（左图位置在258，为方便演示，这里设成和马赛克显示位置一样的坐标）
    var oImg = ctx3.getImageData(start_x-size/2,start_y-size/2,size,size);
    
    var w = oImg.width;
    var h = oImg.height;
    //马赛克的程度，数字越大越模糊
    var num = parseInt(msk_mhcd);
    //等分画布
    var stepW = w/num;
    var stepH = h/num;
    //这里是循环画布的像素点
    for(var i=0;i<stepH;i++){
    	var circle= Math.sqrt((size*size/4-i*i));
        for(var j=0;j<stepW&&j<circle;j++){
    ...
    ctx3.putImageData(oImg,start_x-size/2,start_y-size/2);
}
...
function setXY(obj,x,y,color){
    ...
    var h = obj.height;
    var d = obj.data;
    if(color[0]!=null&&color[1]!=null&&color[2]!=null&&color[3]!=null){
...
$(function() {
	  ...
	  canvas3.onmousedown = function (e) {
			...
			draw(canvas3,location.x,location.y,msk_mhcd,msk_hbdx);
	  };
});